...
function obiettiviPratici(){
  ...
  return [
    {ic:'🔧',t:'Costruisci il primo agente',d:'Un flusso salvato, con trigger e output.',
     val:q('SELECT COUNT(*) c FROM agents WHERE author=?',[io]),obiettivo:1,xp:50,vai:"startNewAgent()"},
    {ic:'▶️',t:'Portalo a termine',d:'Cinque esecuzioni concluse senza errori.',
     val:q("SELECT COUNT(*) c FROM exec_log WHERE status='ok' AND user=?",[io]),obiettivo:5,xp:80,vai:"go('builder')"},
    {ic:'🛡️',t:'Presidia un flusso',d:'Un agente con almeno due controlli: mascheramento, convalida, approvazione o gestione errori.',
     val:conControlli,obiettivo:1,xp:100,vai:"go('builder')"},
    {ic:'📚',t:'Fonda le risposte sui documenti',d:'Almeno tre documenti indicizzati e un recupero riuscito in esecuzione.',
     val:Math.min(q('SELECT COUNT(*) c FROM kb_docs'),3)+(q("SELECT COUNT(*) c FROM execution_events WHERE event_type='kb:retrieved'")?1:0),
     obiettivo:4,xp:120,vai:"openKBDocsModal()"},
    {ic:'🔍',t:'Interroga un risultato',d:'Apri almeno una volta «Perché questo risultato» su un\'esecuzione.',
     val:parseInt(localStorage.getItem('relaition_xai_aperto_'+io)||localStorage.getItem('relaition_xai_aperto')||'0',10),obiettivo:1,xp:60,vai:"go('execlog')"},
    {ic:'🚀',t:'Porta un agente in revisione',d:'Una richiesta di pubblicazione che superi i controlli bloccanti.',
     // publications non porta l'autore: si risale all'agente pubblicato.
     val:q('SELECT COUNT(*) c FROM publications p JOIN agents a ON a.id=p.agent_id WHERE a.author=?',[io]),obiettivo:1,xp:150,vai:"go('marketplace')"},
    {ic:'🎬',t:'Completa il giro guidato',d:'Dodici passaggi sulle schermate reali.',
     val:localStorage.getItem('relaition_tour_fatto_'+io)||localStorage.getItem('relaition_tour_fatto')?1:0,obiettivo:1,xp:50,vai:"startGuida(false)"},
    {ic:'♻️',t:'Chiudi il cerchio',d:'Un agente che produce un file e lo salva nella Knowledge Base.',
     val:q("SELECT COUNT(*) c FROM kb_docs WHERE origin LIKE 'agente:%'"),obiettivo:1,xp:130,vai:"go('builder')"}
  ];
}
...
function renderObiettivi(){
  ...
  return '<div class="card" style="padding:18px;margin-bottom:22px">'+
    ...
    '</div>'+
    '<div style="font-size:12px;color:var(--tx3);margin-bottom:14px">Si verificano da soli sui dati della piattaforma: nessuna spunta manuale, nessun progresso simulato.</div>'+
    ob.map(function(o){
      var fatto=o.val>=o.obiettivo;
      var pct=Math.min(100,Math.round(o.val/o.obiettivo*100));
      return '<div style="display:flex;gap:12px;align-items:center;padding:9px 0;border-bottom:1px solid var(--bg2)">'+
        '<span style="font-size:18px;opacity:'+(fatto?1:.45)+'">'+o.ic+'</span>'+
        '<div style="flex:1;min-width:0">'+
          '<div style="font-size:12.5px;font-weight:700">'+(fatto?'✅ ':'')+escHtml(o.t)+'</div>'+
        ...
        '</div>'+
        (fatto?'':'<button class="tb-btn" style="font-size:11px;flex-shrink:0" onclick="'+o.vai+'">Vai</button>')+
      '</div>';
    }).join('')+
  '</div>';
}
...
var POSTS=[
  ...
  {id:11,user:'Giulia D.',ava:'GD',color:'#F59E0B',title:'Il mio flusso di riconciliazione fatture — scaricabile',body:'Allego il workflow esportato in JSON: trigger su file upload, estrazione dei campi con output vincolato, confronto con l\'ordine a sistema e nota di scostamento. Importatelo dal builder con "Importa agente". Funziona su fatture italiane, per quelle estere va rivisto il prompt di estrazione.',type:'Showcase',tag:'Finance',likes:73,comments:19,views:612,time:'2 giorni fa',
   ...
   attach_data:'data:application/json;base64,'+btoa(JSON.stringify({name:'Riconciliazione fatture',nodes:[{t:'tr',n:'File upload'},{t:'ai',n:'Estrai campi'},{t:'cd',n:'Scostamento > 5%?'},{t:'ou',n:'Esporta file'}],nota:'Esempio dimostrativo allegato al post della Community'}))},
  ...
  document.getElementById('posts-list').innerHTML=list.map(function(p){
    ...
    return '<div class="post-card" onclick="openPostDetail('+p.id+')">'+
      ...
      '<div class="post-actions">'+
        '<span class="post-action" onclick="likePost('+p.id+')">❤️ '+p.likes+'</span>'+
        '<span class="post-action">💬 '+p.comments+'</span>'+
        '<span class="post-action">👁️ '+p.views+'</span>'+
...
function openPostDetail(id){
  ...
  openModal(
    ...
    '<div style="display:flex;gap:16px;padding:12px 0;border-top:1px solid var(--bg2);border-bottom:1px solid var(--bg2);margin-bottom:16px">'+
      '<span class="post-action" onclick="likePost('+p.id+');openPostDetail('+p.id+')">❤️ '+p.likes+' Like</span>'+
      '<span class="post-action">💬 '+comments.length+' Commenti</span>'+
      '<span class="post-action">👁️ '+p.views+' Visualizzazioni</span>'+
    '</div>'+
    '<h3 style="font-size:14px;font-weight:700;margin-bottom:12px">💬 Commenti ('+comments.length+')</h3>'+
    comments.map(function(c){
      return '<div class="comment-item"><div class="comment-ava" style="background:'+c.color+'">'+c.ava+'</div><div class="comment-body"><div class="comment-user">'+c.user+'</div><div class="comment-text">'+c.text+'</div><div class="comment-time">'+c.time+'</div></div></div>';
    }).join('')+
...
function addComment(postId){
  ...
  var p=POSTS.find(function(x){return x.id===postId});if(p)p.comments++;
  saveForumState();
...
function saveProfileData(){
  ...
  if(nuovoNome&&nuovoNome!==vecchioNome){
    ...
    if(quante&&!confirm('Rinominare "'+vecchioNome+'" in "'+nuovoNome+'"?\n\n'+
        ...
        'progressi formativi e XP verranno riattribuit'+(quante===1?'o':'i')+' al nuovo nome.'))return;
