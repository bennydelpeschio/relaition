...
> **Vanno usati con l'app aperta in locale, non con la copia su GitHub Pages.**
> Entrambi i servizi accettano richieste solo da `localhost`, `127.0.0.1` o da
> una pagina aperta con `file://`: una pagina servita da un dominio pubblico
> viene rifiutata, e l'app riporta «servizio non in ascolto» mentre il servizio
> sta rispondendo. Non è un difetto: senza quel filtro qualunque sito aperto
> nello stesso browser potrebbe far inviare email dal tuo computer. Per provare
> l'invio reale, scarica il repository e apri l'app da lì.
