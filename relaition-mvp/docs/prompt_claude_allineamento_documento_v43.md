# Prompt per Claude, allineare il documento alla POC (v43)

Da usare in una **conversazione Claude** (non Claude Code) allegando due file:

1. `relaition-mvp aggiornata 20260912.zip` — la POC completa
...
### Cosa è cambiato nelle ultime tre versioni (10-12 settembre)

Le trovi documentate in `ARCHITECTURE.md` da §5.145 a §5.148, ma te le elenco
perché sono quelle più facili da lasciare indietro nel capitolo 3 e in
appendice:

- **Sandbox didattica**: non permette più di salvare, pubblicare o pianificare
  (solo esportare). Se il capitolo la descrive come «un Builder con palette
  ridotta», va aggiunto che è anche senza persistenza per scelta.
- **Aiuto alla scrittura** nei campi di testo (prompt dei nodi AI, contesto del
  flusso, testi dei nodi): il modello collegato scrive o migliora il testo a
  partire da un obiettivo detto a parole. È una funzione nuova, con un modulo
  proprio (`js/aiuto-testo.js`): probabilmente non compare da nessuna parte nel
  documento.
- **Chat del Builder e etichette delle frecce su più righe.**
- **Pulsante «Impara»** nella barra in alto, ingresso diretto al Learning Hub.
- **Registro di esecuzione**: le righe si espandono e il testo intero non viene
  più tagliato alla fonte (tetto dichiarato a 4000 caratteri).
- **Demo guidata**: 97 passi in 10 capitoli, con un capitolo finale che esce e
  rientra con un altro account per mostrare la separazione dei dati per
  persona; sezione Sfide con iscrizione, candidatura, classifica e domande.
- **Dati**: le iscrizioni alle sfide vengono ricostruite dalle candidature
  (migrazione in `js/db.js`); i livelli del Learning Hub sono AI Aspirant,
  AI Translator, AI Creator, AI Ambassador.
- **Tipografia**: niente trattini lunghi nei testi visibili, niente emoji nei
  titoli. Gli screenshot della tesi, oltre ai colori, mostrano titoli con
  icone che non ci sono più: altro motivo per rifarli.
- **Esempio in più**: `esempi/4-invoice-extractor.json`, che corrisponde alla
  tabella 3.H.2 del capitolo (Invoice Extractor, Finance). Verifica che la
  tabella e il flusso coincidano passo per passo, e che il documento citi il
  file in appendice.
